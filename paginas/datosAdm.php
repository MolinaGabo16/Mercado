...
if($consulta=$conexion->query($sql)){
    if($consulta->num_rows > 0){
            ...
            while($fila = $consulta->fetch_assoc()){
                  $id=$fila['id_p'];
                  $Boton1="<td><a href=borrar_pregAdm.php?idP=$id> <button class='boton naranja'> Marcar Vendido </button></a></td>";
                  $Boton2="<td><a href=comentarAdm.php?idP=$id> <button type='button' > Ver Comentarios </button></a></td>";
                  $Boton3="<td><a href=editarUserAdm.php?idP=$id> <button type='button' > Editar Venta </button></a></td>";
                  $id_img=$fila['imagen'];
